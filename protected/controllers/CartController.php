...
Class CartController extends YiishopController {
	...
	function actionFinishshop() {
		...
		if(!empty($_POST['kode_member'])){
			$kodemember=$_POST['kode_member'];
		}else{
			$m=Member::model()->findByAttributes(array('id'=>Yii::app()->user->id));
			$kodemember=$m->kode_member;
			//echo $kodemember;
		}
		//echo $kodemember;
		
		if($this -> addOrderCleanCart($kodemember)){
	...
	private function addOrderCleanCart($kodemember) {
		$modelOrder = new Order;
	if(isset($_POST)){
		// 	$transaction = Yii::app()->db->beginTransaction();
		// try{
			$modelOrder -> order_code = $order_code = $kodemember . '' . $this -> orderCode();
			$modelOrder -> order_date = $this->date_sql_now();
			$modelOrder -> kode_member = $kodemember;
			$modelOrder -> bank_transfer ='BCA-09123123123';
			//print_r($modelOrder);
                        
			if ($modelOrder -> save()) {
			$last_insert_id = Yii::app() -> db -> getLastInsertID();
			$sql = "SELECT p.id as proid, p.image,p.harga,p.nama_produk,cart.* 
					  FROM cart,product p
					  WHERE p.id=cart.product_id
					  AND cart.cart_code='" . Yii::app() -> session['cart_code'] . "'";
